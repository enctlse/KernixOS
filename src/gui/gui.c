...
void gui_move_window(gui_window_t* window, int x, int y) {
    ...
    window->x = x;
    window->y = y;
    gui_mark_dirty(old_x, old_y, window->width, window->height);
    gui_mark_dirty(window->x, window->y, window->width, window->height);
}
...
void gui_draw_taskbar() {
    ...
    int time_x = fb_w - str_len(time_str) * 8 - 10;
    int time_y = fb_h - TASKBAR_HEIGHT + 5 + 5;
    gui_draw_text_fast(time_str, time_x, time_y, 0xFF000000);
}
void gui_draw_window(gui_window_t* window) {
    ...
    int bg_x, bg_y, bg_w, bg_h, window_content_x, window_content_y;
    if (str_equals(window->title, "Start Menu")) {
        bg_x = content_x + 1;
        bg_y = content_y + 1;
        bg_w = content_w - 2;
        bg_h = content_h - 2;
        window_content_x = bg_x + 10;
        window_content_y = bg_y + 10;
    } else {
        ...
        draw_rect(title_x, title_y, title_w, 1, 0xFFFFFFFF);
        draw_rect(title_x, title_y, 1, title_h, 0xFFFFFFFF);
        draw_rect(title_x + title_w - 1, title_y, 1, title_h, 0xFF000000);
        draw_rect(title_x, title_y + title_h - 1, title_w, 1, 0xFF000000);
        if (window->focused) {
            for (int i = 0; i < 3; i++) {
                draw_rect(title_x + 1, title_y + title_h - 1 - i, title_w - 2, 1,
                          (title_color & 0xFFFFFF00) | ((title_color & 0xFF) - i * 20));
            }
        }
        draw_rect(title_x, title_y + title_h, title_w, 1, 0xFF000000);
        bg_x = content_x + 1;
        bg_y = content_y + WINDOW_TITLE_HEIGHT + 1;
        bg_w = content_w - 2;
        bg_h = content_h - WINDOW_TITLE_HEIGHT - 2;
    ...
    if (str_equals(window->title, "Terminal")) {
        ...
        draw_rect(window->x, window->y, window->width, window->height, 0xFFFFFFFF);
        draw_rect(window->x + 1, window->y + 1, window->width - 2, window->height - 2, 0xFF808080);
...
static void on_start_button_click(gui_button_t* button) {
    ...
    u32 fb_h = get_fb_height();
    gui_window_t* start_window = gui_create_window("Start Menu", 10, fb_h - TASKBAR_HEIGHT - 250, 200, 250);
    if (start_window) {
