...
static void load_limine_module(void) {
    ...
    for (u64 i = 0; i < response->module_count; i++) {
        ...
        if (fd < 0) {
            SYSTEM_PRINT("  ERROR: cannot create ", theme_red);
            SYSTEM_PRINT(vfs_path, theme_white);
            SYSTEM_PRINT("\n", theme_white);
            continue;
        }
        ...
        if (written > 0) {
            ...
            SYSTEM_PRINT(module->path, theme_white);
            SYSTEM_PRINT(" : ", theme_white);
            SYSTEM_PRINT(vfs_path, theme_white);
            str_copy(buf, " with ");
            str_append_uint(buf, (u32)written);
            str_append(buf, " bytes\n");
            SYSTEM_PRINT(buf,  theme_white);
        }
    }
}
void fs_system_init(void *kernel_memory)
{
    ...
    SYSTEM_PRINTF("[FS] wrote %s \n", logpath);
    if (init_boot_log < 0) {
        initiate_panic("Cannot open [logs]");
    }
...
{
    int total = count_components();
    int dev_cnt = 0;
    SYSTEM_PRINT("[FS] ", gray_70);
    SYSTEM_PRINT("scann modules:\n", theme_white);
    for (int i = 0; i < total; i++) {
        struct component_handler *handler = component_at_position(i);
        if (handler && handler->attachment_point && str_starts_with(handler->attachment_point, _DEV)) {
            if (devfs_register_device(handler) == 0) {
                dev_cnt++;
            }
        }
    }
    ...
    SYSTEM_PRINT_INT(dev_cnt, theme_white);
...
void fs_create_test_file(void) {
    ...
    if (fd < 0) {
        SYSTEM_PRINT("[FS] ", gray_70);
        SYSTEM_PRINT("failed to create file\n", theme_white);
        return;
    }
    fs_close(fd);
    SYSTEM_PRINT("[FS] ", gray_70);
    SYSTEM_PRINT("created ", theme_white);
    SYSTEM_PRINT("/tmp/t", theme_white);
    SYSTEM_PRINT("\n", theme_white);
}