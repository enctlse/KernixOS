...
static void usb_mouse_interrupt_handler(uint8_t* report, int report_size) {
    ...
    if (delta_x > 10 || delta_x < -10) delta_x *= 2;
    if (delta_y > 10 || delta_y < -10) delta_y *= 2;
    usb_mouse_x_accum += delta_x * sensitivity;
    usb_mouse_y_accum += delta_y * sensitivity;
    int32_t next_x = usb_mouse_x_accum / 100;
    int32_t next_y = usb_mouse_y_accum / 100;
    ...
    if (next_x >= (int32_t)(fb_width - USB_MOUSE_CURSOR_WIDTH)) {
        next_x = fb_width - USB_MOUSE_CURSOR_WIDTH - 1;
        usb_mouse_x_accum = next_x * 100;
    }
    if (next_y >= (int32_t)(fb_height - USB_MOUSE_CURSOR_HEIGHT)) {
        next_y = fb_height - USB_MOUSE_CURSOR_HEIGHT - 1;
        usb_mouse_y_accum = next_y * 100;
    }
    usb_mouse_x = next_x;
    usb_mouse_y = next_y;
...
void usb_mouse_simulate_interrupt() {
    ...
    if (usb_mouse_test_mode) {
        if (counter % 50 == 0) {
            ...
            usb_mouse_x_accum = usb_mouse_x * 100;
            usb_mouse_y_accum = usb_mouse_y * 100;
            if (usb_mouse_callback) {
                usb_mouse_callback(usb_mouse_x, usb_mouse_y, usb_mouse_buttons);
            }
        }
...
void usb_mouse_set_position(int32_t x, int32_t y) {
    usb_mouse_x = x;
    usb_mouse_y = y;
    usb_mouse_x_accum = x * 100;
    usb_mouse_y_accum = y * 100;
}
