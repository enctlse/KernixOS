...
void e1000_init(uint32_t io_base) {
    ...
    e1000_write_reg(E1000_REG_RCTL, 0x400801C2);
    ...
    e1000_write_reg(E1000_REG_TCTL, 0x4010A);
    e1000_write_reg(E1000_REG_TIPG, 0x60100A);
...
void e1000_send_packet(void* data, int len) {
    ...
    e1000_dev.tx_descs[cur].cmd = 0xB;
...
int e1000_receive_packet(void* buffer, int max_len) {
    ...
    if (!(e1000_dev.rx_descs[cur].status & 0x1)) return 0;
