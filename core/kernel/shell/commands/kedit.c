...
static void kedit_draw() {
    ...
    // Добавляем пустую строку перед началом текста для отступа
    u32 line_spacing = 1; // Дополнительный отступ между строками

    for (int i = 0; i < line_count; i++) {
        cursor_x = 0;
        // Добавляем дополнительное пространство между строками
...
static void kedit_save() {
    ...
    for (int i = 0; i < line_count; i++) {
        ...
        if (i < line_count - 1) { // Не добавляем перевод строки после последней строки
            fs_write(fd, "\n", 1);
        }
    }
    ...
    // Очищаем строку и показываем сообщение
    cursor_x = 0;
...
static void kedit_insert_char(char c) {
    ...
    if (len >= MAX_LINE_LEN - 1) return; // Проверка на переполнение строки
...
static void kedit_new_line() {
    ...
    if (edit_cursor_x <= current_len) {
        ...
        // Копируем оставшуюся часть текущей строки в новую
...
void kedit_handle_key(int key) {
    // Для отладки: распечатать код клавиши
    // char debug[32];
    // str_copy(debug, "Key: ");
    // str_append_int(debug, key);
    // str_append(debug, "\n");
    // print(debug, yellow);
    ...
    if (key == '\n' || key == '\r') {
        kedit_new_line();
    } else if (key == 8 || key == 127) { // Backspace (оба варианта)
        kedit_delete_char();
    } else if (key >= 32 && key <= 126) {
        kedit_insert_char((char)key);
    } else if (key == 0x80) { // Up arrow
        kedit_move_cursor(0, -1);
    } else if (key == 0x81) { // Down arrow
        kedit_move_cursor(0, 1);
    } else if (key == 0x82) { // Left arrow
        kedit_move_cursor(-1, 0);
    } else if (key == 0x83) { // Right arrow
        kedit_move_cursor(1, 0);
    } else {
...
FHDR(cmd_kedit) {
    ...
    // Очищаем экран и скрываем приглашение
    shell_clear_screen(CONSOLESCREEN_BG_COLOR);
    banner_force_update();
