...
void smp_init(void) {
    ...
    if (mp_request.response && mp_request.response->cpu_count > 0) {
        ...
        for (u64 i = 0; i < mp_request.response->cpu_count; i++) {
            ...
            BOOTUP_PRINT("[SMP] CPU ", gray_70);
            BOOTUP_PRINT_INT(i, cyan);
            BOOTUP_PRINT(" APIC ID ", gray_70);
            BOOTUP_PRINT_INT(cpu->lapic_id, cyan);
            BOOTUP_PRINT(" Processor ID ", gray_70);
            BOOTUP_PRINT_INT(cpu->processor_id, cyan);
            BOOTUP_PRINT("\n", gray_70);
    ...
    BOOTUP_PRINT("[SMP] Detected ", gray_70);
    BOOTUP_PRINT_INT(cpu_count, cyan);
    BOOTUP_PRINT(" CPUs\n", gray_70);
}
