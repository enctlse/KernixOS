...
static void* acpi_find_table(const char* signature) {
    ...
    if (rsdp->revision >= 2 && rsdp->xsdt_address) {
        BOOTUP_PRINT("[ACPI] Trying XSDT at 0x", gray_70);
        BOOTUP_PRINT_INT(rsdp->xsdt_address >> 32, cyan);
        BOOTUP_PRINT_INT(rsdp->xsdt_address & 0xFFFFFFFF, cyan);
        BOOTUP_PRINT("\n", gray_70);
        int xsdt_valid = 0;
        ...
        if (!xsdt_valid) {
            ...
            BOOTUP_PRINT("[ACPI] XSDT entry count: ", gray_70);
            BOOTUP_PRINT_INT(entry_count, cyan);
            BOOTUP_PRINT("\n", gray_70);
            for (u32 i = 0; i < entry_count; i++) {
                BOOTUP_PRINT("[ACPI] Checking XSDT entry ", gray_70);
                BOOTUP_PRINT_INT(i, cyan);
                BOOTUP_PRINT("\n", gray_70);
    ...
    if (rsdp->rsdt_address) {
        BOOTUP_PRINT("[ACPI] Trying RSDT at 0x", gray_70);
        BOOTUP_PRINT_INT(rsdp->rsdt_address >> 32, cyan);
        BOOTUP_PRINT_INT(rsdp->rsdt_address & 0xFFFFFFFF, cyan);
        BOOTUP_PRINT("\n", gray_70);
        int rsdt_valid = 0;
        if (memmap_request.response) {
            for (u64 i = 0; i < memmap_request.response->entry_count; i++) {
        ...
        if (!rsdt_valid) {
            ...
            BOOTUP_PRINT("[ACPI] RSDT entry count: ", gray_70);
            BOOTUP_PRINT_INT(entry_count, cyan);
            BOOTUP_PRINT("\n", gray_70);
            for (u32 i = 0; i < entry_count; i++) {
                BOOTUP_PRINT("[ACPI] Checking RSDT entry ", gray_70);
                BOOTUP_PRINT_INT(i, cyan);
                BOOTUP_PRINT("\n", gray_70);
...
int acpi_init(void) {
    ...
    BOOTUP_PRINT("[ACPI] RSDP address: 0x", gray_70);
    BOOTUP_PRINT_INT((u64)rsdp_request.response->address >> 32, cyan);
    BOOTUP_PRINT_INT((u64)rsdp_request.response->address & 0xFFFFFFFF, cyan);
    BOOTUP_PRINT("\n", gray_70);
    int address_valid = 0;
    if (memmap_request.response) {
        for (u64 i = 0; i < memmap_request.response->entry_count; i++) {
    ...
    while (entry_ptr < madt_end) {
        ...
        if (entry->type == MADT_TYPE_LOCAL_APIC) {
            ...
            if (lapic->flags & 1) {
                ...
                BOOTUP_PRINT_INT(lapic->processor_id, cyan);
                BOOTUP_PRINT(" APIC ID ", gray_70);
                BOOTUP_PRINT_INT(lapic->apic_id, cyan);
                BOOTUP_PRINT("\n", gray_70);
            }
        }
