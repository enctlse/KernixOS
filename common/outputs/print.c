...
static inline void putchar_fast(char c, u32 x, u32 y, u32 color)
{
    ...
    if (font_scale == 1) {
        for (u32 glyph_y = 0; glyph_y < glyph_height; glyph_y++) {
            ...
            if (!row) continue;
            u32* dest = &buf[(y + glyph_y) * stride + x];
        ...
        for (u32 glyph_y = 0; glyph_y < glyph_height; glyph_y++) {
            ...
            for (u32 scale_y = 0; scale_y < font_scale; scale_y++) {
                u32 screen_y = y + glyph_y * font_scale + scale_y;
                ...
                for (u32 glyph_x = 0; glyph_x < glyph_width; glyph_x++) {
                    if (row & (0x80 >> glyph_x)) {
                        for (u32 scale_x = 0; scale_x < font_scale; scale_x++) {
                            u32 pixel_x = glyph_x * font_scale + scale_x;
                            if (x + pixel_x < get_fb_width()) {
                                dest[pixel_x] = color;
                            }
                        }
                    }
...
void putchar(char c, u32 color)
{
    ...
    if (c == '\t') {
        for (int tab_count = 0; tab_count < 4; tab_count++) {
...
void print_hex(u32 num, u32 color)
{
    ...
    for (int digit_index = 0; digit_index < 8; digit_index++) {
        buffer[2 + digit_index] = hex_digits[(num >> ((7 - digit_index) * 4)) & 0xF];
    }
    buffer[10] = '\0';
    string(buffer, color);
}
void print_hex64(u64 num, u32 color)
{
    ...
    buffer[0] = '0';
    buffer[1] = 'x';
    for (int digit_index = 0; digit_index < 16; digit_index++) {
        buffer[2 + digit_index] = hex_digits[(num >> ((15 - digit_index) * 4)) & 0xF];
    }
    buffer[18] = '\0';
    string(buffer, color);
}
